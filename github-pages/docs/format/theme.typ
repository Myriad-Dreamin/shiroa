...
To apply set rules for different targets, your `template.typ` can import and respect the `x-target` variable from `@preview/shiroa:0.4.0`. For example, to remove margins for web target, you can do:
...
```typ
#import "@preview/shiroa:0.4.0": page-width, x-target
